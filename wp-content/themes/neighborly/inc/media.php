...
/** This is the original Hybrid Media Grabber script graciously made available by Justin Tadlock
 * (https://github.com/justintadlock/hybrid-core/blob/master/classes/hybrid-media-grabber.php) 
 * with all the original copyright information left intact.
 * 
 * The only thing I changed on this script was the inclusion of a theme_slug in the naming of the functions.
 * The reason for this is the fact that the WordPress Theme Review Team has a stipulation that all
 * themes are required to use a theme-slug ( or a reasonably unique slug) as a prefix for anything 
 * in the public namespace, including all custom function names, classes, hooks, public/global variables,
...
/**
 * Wrapper function for the neighborly_first_media class.  Returns the HTML output for the found media.
 *
 * @since  0.1.0
 * @access public
 * @param  array
 * @return string
 */
function neighborly_first_media( $args = array() ) {

	$media = new neighborly_first_media( $args );
...
class neighborly_first_media {
	...
	public function __construct( $args = array() ) {
		...
		add_filter( 'neighborly_first_media_embed_shortcode_media', array( $wp_embed, 'run_shortcode' ) );
		add_filter( 'neighborly_first_media_autoembed_media',       array( $wp_embed, 'autoembed' ) );
		...
		$this->args    = apply_filters( 'neighborly_first_media_args', wp_parse_args( $args, $defaults ) );
	...
	public function get_media() {
		return apply_filters( 'neighborly_first_media_media', $this->media, $this );
	}
	...
	public function do_embed_shortcode_media( $shortcode ) {
		...
		$this->media = apply_filters(
			'neighborly_first_media_embed_shortcode_media',
			$this->original_media
		);
	}
	...
	public function do_autoembed_media() {
		...
		if ( is_array( $matches ) ) {
			...
			foreach ( $matches as $value ) {
				...
				$embed = trim( apply_filters( 'neighborly_first_media_autoembed_media', $value[0] ) );
	...
	public function filter_dimensions( $html ) {
		...
		list( $width, $height ) = apply_filters( 
			'neighborly_first_media_dimensions', 
