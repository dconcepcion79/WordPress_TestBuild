	...
	if( !function_exists('gdlr_get_blog_info') ){
		function gdlr_get_blog_info( $array = array(), $wrapper = true, $sep = '' ){
			...
			if( !empty($theme_option['new-fontawesome']) && $theme_option['new-fontawesome'] == 'enable' ){
				$icon_class = array('date'=>'fa fa-calendar-o', 'tag'=>'fa fa-tags', 'category'=>'fa fa-folder-o', 
					'comment'=>'fa fa-comment-o', 'author'=>'fa fa-pencil');
			}else{
				$icon_class = array('date'=>'icon-calendar-empty', 'tag'=>'icon-tags', 'category'=>'icon-folder-close-alt', 
			...
			foreach($array as $post_info){
				...
				switch( $post_info ){
					...
					case 'comment':
						ob_start();
						comments_popup_link( 
							__('0 Comments', 'gdlr_translate'), 
							__('1 Comment', 'gdlr_translate'), 
							__('% Comments', 'gdlr_translate'), 
							__('comments-link', 'gdlr_translate'), 
