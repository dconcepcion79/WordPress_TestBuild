	...
	*/
	...
	if( !function_exists('gdlr_get_item_title') ){
		function gdlr_get_item_title( $atts ){
			...
			if( !empty($atts['title-type']) && $atts['title-type'] != 'none' ){
				...
				// for left and left divider style
				if( $atts['title-type'] == 'left' || $atts['title-type'] == 'left-divider' ){
				...
				// exclude left and left divider text link out
				if( !empty($atts['carousel']) || 
					( !empty($atts['right-text']) && !empty($atts['right-text-link']) && 
						($atts['title-type'] != 'left' && $atts['title-type'] != 'left-divider') ) ){
					$ret .= '<div class="gdlr-item-title-right">';
					if( !empty($atts['port-filter']) ){
						$ret .= $atts['port-filter'];
					}
					if( !empty($atts['right-text']) && !empty($atts['right-text-link']) && 
						($atts['title-type'] != 'left' && $atts['title-type'] != 'left-divider') ){
						$ret .= '<a class="gdlr-item-title-link" href="' . esc_url($atts['right-text-link']) . '" >' . $atts['right-text'] . '<i class="fa fa-angle-double-right icon-double-angle-right"></i></a>';
					}
					if( !empty($atts['carousel']) ){
	...
	if( !function_exists('gdlr_get_column_service_item') ){
		function gdlr_get_column_service_item( $settings ){
			...
			if( empty($settings['type']) || $settings['type'] == 'image' ){
				...
				$ret .= '<div class="column-service-icon gdlr-skin-box">';
			...
			if( !empty($settings['learn-more-link']) ){
				$ret .= '<div class="column-service-link" >';
				$ret .= '<a href="' . esc_attr($settings['learn-more-link']) . '" >';
				$ret .= __('Learn More', 'gdlr_translate');
	...
	if( !function_exists('gdlr_get_stunning_text_item') ){
		function gdlr_get_stunning_text_item( $settings ){
			...
			$ret .= '<div class="gdlr-item gdlr-stunning-text-item" ' . $item_id . $margin_style . '>';
			$ret .= '<div class="gdlr-stunning-text-overlay" ></div>';
			$ret .= '<h2 class="stunning-text-title">' . gdlr_text_filter($settings['title']) . '</h2>';
			if( !empty($settings['button-link']) ){
				$ret .= '<a class="stunning-text-button" href="' . esc_url($settings['button-link']) . '" target="_blank" >';
				$ret .= $settings['button-text'];
				$ret .= '<i class="fa fa-angle-double-right icon-double-angle-right"></i>';
				$ret .= '</a>';
			}
