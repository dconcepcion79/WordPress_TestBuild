<?php
	...
	function su_relative_time( $original, $do_more = 0 ) {
		...
		$chunks = array(
			array( 60 * 60 * 24 * 365, __( 'year', 'care' ) ),
			array( 60 * 60 * 24 * 30, __( 'month', 'care' ) ),
			array( 60 * 60 * 24 * 7, __( 'week', 'care' ) ),
			array( 60 * 60 * 24, __( 'day', 'care' ) ),
			array( 60 * 60, __( 'hour', 'care' ) ),
			array( 60, __( 'minute', 'care' ) ),
		);
		...
		$return = ($count == 1) ? '1 ' . $name : "$count {$name}" . __( 's', 'care' );
		...
		if ( $i + 1 < $j ) {
			...
			if ( (($count2 = floor( ($since - ($seconds * $count)) / $seconds2 )) != 0) && $do_more )
				$return .= ( $count2 == 1) ? ', 1 ' . $name2 : ", $count2 {$name2}" . __( 's', 'care' );
		}
		return $return;
	}
	...
	function su_get_tweets( $username, $limit, $show_time ) {
		...
		if ( $username == '' ) {
			$return = '<p class="su-error"><strong>Tweets:</strong> ' . __( 'username not specified', 'care' ) . '&hellip;</p>';
		} else {
			if ( empty( $messages->items ) ) {
				$return = '<p class="su-error"><strong>Tweets:</strong> ' . __( 'no public messages', 'care' ) . '&hellip;</p>';
			} else {
				$i = 0;
