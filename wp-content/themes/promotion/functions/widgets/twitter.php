...
if (!class_exists('Twitter_Widget')) :
	class Twitter_Widget extends WP_Widget {
		...
		function Twitter_Widget() {
			...
			// Create the widget
			$this->WP_Widget('twitter-widget', 'CUSTOM - Twitter', $widget_ops);
		}
		...
		function widget($args, $instance) {
			...
			if (!file_exists($cachefile) || (file_exists($cachefile) && (filemtime($cachefile) + $interval) < time())) :
				...
				$feed = fetch_feed('https://twitter.com/statuses/user_timeline.rss?screen_name=' . $username);
				...
				if (method_exists($feed, 'get_items')) :
					...
					foreach	($tweets as $t) :
						...
						$result .= '
								<span class="twitter-text">' . substr($text, $prefixlen, strlen($text) - $prefixlen) . '</span>' . ($datebreak ? '<br />' : '');
								
						// Display date/time
						if ($datedisplay) $result .= '
								<span class="twitter-date"><a href="'. $t->get_permalink() .'">' . $time . '</a></span>';
...
?>
			...
			<p>
				...
				<label for="<?php echo $this->get_field_id('datebreak'); ?>">Add linebreak befor date?</label>
