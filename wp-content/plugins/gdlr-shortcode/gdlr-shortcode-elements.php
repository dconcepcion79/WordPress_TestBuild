<?php
	...
	function gdlr_stunning_text_shortcode( $atts, $content = null ){
		...
		$ret  = '<div class="gdlr-stunning-shortcode-ux gdlr-ux">';
		$ret .= '<div class="gdlr-stunning-shortcode-item ' . $stunning_class . '" ' . $stunning_style . ' >';
		if(!empty($title)){
			$stunning_style = empty($title_color)? '': 'style="color: ' . $title_color . '" ';
			$ret .= '<h2 class="stunning-shortcode-title" ' . $stunning_style . '>' . $title . '</h2>';
		}
		if(!empty($content)){
			$stunning_style = empty($caption_color)? '': 'style="color: ' . $caption_color . '" ';
			$ret .= '<div class="stunning-shortcode-caption" ' . $stunning_style . '>' . $content . '</div>';
		}
		if(!empty($button) && !empty($button_link)){
			...
			$ret .= '<a class="stunning-shortcode-button gdlr-button " href="' . $button_link . '" ' . $stunning_style . '>';
			$ret .= $button;
			$ret .= '</a>';
		}
		$ret .= '</div>'; // gdlr-item
	...
	// box icon shortcode
	add_shortcode('gdlr_service', 'gdlr_service_shortcode');
	function gdlr_service_shortcode( $atts, $content = null ){
		extract( shortcode_atts(array('image'=>'', 'title'=>'', ), $atts) );
		
		$ret  = '<div class="gdlr-service-shortcode">';
		if( !empty($image) ){
			$ret .= '<div class="gdlr-service-shortcode-image" >';
			$ret .= '<img src="' . $image . '" alt="" />';
			$ret .= '</div>';
		}
		$ret .= '<div class="gdlr-service-shortcode-content-wrapper" >';
		$ret .= '<h3 class="gdlr-service-shortcode-title">' . $title . '</h3>';
		$ret .= '<div class="gdlr-service-shortcode-content">' . $content . '</div>';
		$ret .= '<div class="clear"></div>';
		$ret .= '</div>';
		$ret .= '</div>';
